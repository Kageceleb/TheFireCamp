...
6. ~~Symbiote integration~~ — done: TaleSpire's type definitions, the
   ...
   plain browser tab.
7. **DM catalog UI** — done: Items, Spells, Classes tabs, DM-only,
   backed by real Postgres RLS.
8. **Bag of Holding + loot claiming** — done: drop routes to it instead
   of destroying items, DM can spawn/claim-toggle loot, players claim
   with room found automatically.
9. **Journal & Handouts — done this session.** A "Journal" screen
   (visible to everyone, next to Catalog and Bag of Holding) with six
   tabs: Quests, NPCs, World Codex, Locations, Personal Notes, and
   Handouts. The four standard tabs share one component; Personal Notes
   is deliberately separate since its privacy rule needs an authoring
   character to even work (a note with no author could never be matched
   back to its writer for the RLS check to let them see it again).
   Handouts is a DM staging gallery with reveal/hide — **the "Broadcast
   to Party" push notification from spec Module 6 is deliberately not
   built**; revealing something shows up next time a player loads that
   tab, same as everywhere else in this app, rather than an instant
   popup. Same reasoning for handout images: **paste-a-URL only, no file
   upload** — building real upload + client-side webp compression needs
   Supabase Storage, which nothing else in this app uses yet, and a
   rushed half-version felt worse than being upfront that it's not here.
   **Remaining: the Banquet/rest event** — you wanted to think that one
   through more before building it.
10. **Item interaction overhaul — done this session.** Three fixes from
    your review: (1) Equip/Give/Drop now render as a popover anchored
    directly to the selected item itself (`ItemActionPopover.tsx`), not
    a panel at the bottom of the page — both `PocketGrid` and
    `UniformSlotPocket` were restructured to own their own selection
    state's UI instead of reporting up to one shared bottom panel.
    (2) "Give to someone" — a DM's give is immediate (they already have
    write access to any character); a player's give creates an async
    request using the `item_transfers` table from the original schema,
    which the recipient sees in a "Pending Transfers" section and
    accepts or declines. (3) "Take"/"throw on the ground" — the existing
    Drop action already routes to the Bag of Holding and already worked
    for a DM acting on any character's items (DM has `canEdit` on every
    character sheet), so that one didn't need new code, just confirming
    it already did what was asked.

## A note on this session specifically

Partway through building the above, this sandbox's underlying container
got reset, which wiped the working project files entirely (a fresh
`/home/claude` with nothing in it). I was able to recover by unzipping
the **last delivered zip** (from the d20 icon change) as a known-good
base and reapplying this session's changes on top of it from what I
still had in context, then re-verified the whole chain (prop threading,
foreign key constraint names, import paths) before packaging — rather
than trying to reconstruct everything from memory and hoping it was
right. Mentioning this mainly so the reasoning behind that verification
pass makes sense if you're skimming the diffs.
